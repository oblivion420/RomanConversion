...
# compatible for tailo tone 
tailo_tone = "áàâāa̍a̋ÁÀÂĀA̍A̋éèêēe̍e̋ÉÈÊĒE̍E̋íìîīi̍ı̍i̋ÍÌÎĪI̍I̋óòôōo̍őÓÒÔŌO̍Őúùûūu̍űÚÙÛŪU̍Űḿm̀m̂m̄m̍m̋ḾM̀M̂M̄M̍M̋ńǹn̂n̄n̍n̋ŃǸN̂N̄N̍N̋"
re_eng = re.compile(f'[a-zA-Z0-9{tailo_tone}]', re.U)
...
#                return
    ...
    def cut(self, sentence, cut_all=False, HMM=True):
        ...
        blocks = re.split(" ", sentence)
        for blk in blocks:
            if not blk:
                continue
            # if re_han.match(blk):
            if not re_skip_default.fullmatch(blk) or not blk != "": 
