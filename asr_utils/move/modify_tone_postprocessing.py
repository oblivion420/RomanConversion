...
from lib import get_colloquial_tone, get_colloquial_tone_specialcase

cfg = import_module("config.{}".format("db"))

def parse_item(mydb, item, source):

    id, hanzi, tl_standard, tl_collo = item
    ...
    for tailo in tailo_list:
        pinyin_set = set(get_colloquial_tone_specialcase.TAILO(hanzi, tailo).modify())
...
if __name__ == "__main__":
    ...
    result = mydb.select("standard_dictionary", condition_str="檢視狀態='N' and 漢字 LIKE '%仔%' ",
                                                retrieve_str="ID, `漢字`, `TL-標準`, `TL-口語`")
    for item in result:
